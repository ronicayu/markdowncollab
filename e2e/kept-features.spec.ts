...
const TEST_EMAIL = `kept_${Date.now()}@example.com`;
const TEST_PASSWORD = "TestPassword123!";

test.describe("Kept Features (post-2026-04-20 cut)", () => {
  test.setTimeout(60_000);

  test.beforeAll(async ({ browser }) => {
    await resetDatabase();
    const page = await browser.newPage();
    await registerTestUser(page, "KeptTestUser", TEST_EMAIL, TEST_PASSWORD);
    await page.close();
  });
